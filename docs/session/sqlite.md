...
GoMax создаёт таблицу `max_sessions_v2` автоматически и импортирует доступные
строки из старой `max_sessions`. SQLite-драйвер нужно добавить в приложение
самостоятельно; библиотека намеренно не выбирает драйвер.

## Поля

Хранятся `phone`, `token`, `device_id`, `mt_instance_id`, четыре sync-маркера,
`config_hash` и полный `user_agent`. Первичный ключ — token, поэтому одна база
может содержать несколько аккаунтов.
