...
// JoinGroup is an alias for JoinChat specifically for groups.
func (s *ChatService) JoinGroup(ctx context.Context, link string) (*types.Chat, error) {
	return s.JoinChat(ctx, link)
}

// JoinChannel is an alias for JoinChat specifically for channels.
func (s *ChatService) JoinChannel(ctx context.Context, link string) (*types.Chat, error) {
	return s.JoinChat(ctx, link)
}

// ResolveGroupByLink resolves group information by invite link without joining.
func (s *ChatService) ResolveGroupByLink(ctx context.Context, link string) (*types.Chat, error) {
	...
	res, err := s.invoker.Invoke(ctx, protocol.OpChatCheckLink, payload)
	if err != nil {
		return nil, fmt.Errorf("resolve group by link failed: %w", err)
	}

	chat := &types.Chat{InviteLink: link}
	if chatData, ok := res["chat"].(map[string]interface{}); ok {
		if id, ok := chatData["id"].(int64); ok {
			chat.ID = id
		} else if idF, ok := chatData["id"].(float64); ok {
			chat.ID = int64(idF)
		}
		if title, ok := chatData["title"].(string); ok {
			chat.Title = title
...
// GetChats retrieves chat metadata for multiple chat IDs.
func (s *ChatService) GetChats(ctx context.Context, chatIDs []int64) ([]types.Chat, error) {
	...
	var chats []types.Chat
	if cList, ok := res["chats"].([]interface{}); ok {
...
// GetChat retrieves a single chat metadata by ID.
...
// ChangeGroupProfile updates group title, description, and avatar photo.
...
// GetJoinRequests retrieves pending join requests for a private chat.
func (s *ChatService) GetJoinRequests(ctx context.Context, chatID int64, count int) ([]types.Member, error) {
	...
	var members []types.Member
	if mList, ok := res["members"].([]interface{}); ok {
		for _, item := range mList {
			if mData, ok := item.(map[string]interface{}); ok {
				var m types.Member
				if uid, ok := mData["userId"].(int64); ok {
					m.UserID = uid
				} else if uidF, ok := mData["userId"].(float64); ok {
					m.UserID = int64(uidF)
				}
...
// ConfirmJoinRequests approves pending join requests.
func (s *ChatService) ConfirmJoinRequests(ctx context.Context, chatID int64, userIDs []int64) error {
	...
	_, err := s.invoker.Invoke(ctx, protocol.OpChatMembers, payload)
	if err != nil {
		return fmt.Errorf("confirm join requests failed: %w", err)
	}
	return nil
}

// DeclineJoinRequests declines pending join requests.
func (s *ChatService) DeclineJoinRequests(ctx context.Context, chatID int64, userIDs []int64) error {
	...
	_, err := s.invoker.Invoke(ctx, protocol.OpChatMembers, payload)
	if err != nil {
		return fmt.Errorf("decline join requests failed: %w", err)
	}
	return nil
}
